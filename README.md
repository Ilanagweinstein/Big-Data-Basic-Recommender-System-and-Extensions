...
### Overview
In this final project, we applied the big data tools from class to build and evaluate a recommender system using Spark's MLib ALS algorithm. The results were benchmarked with Spark's LightFM algorithm. To understand model performance, visualizations were made with t-SNE. 

### File Descriptions

`statistics_results.txt`: Summary statistics commands for the goodreads_interactions data set. 

`splitting_scripts.py`: Takes a percent of the original Goodreads dataset and splits according to the project guidelines. 

`tune_ALS.py`: Performs hyperparameter tuning on ALS recommendation system.

`evaluations.py`: Evaluate MAP for ALS model in 2 ways.

`extension1.py`: Tunes, evaluates and compares LightFM model to ALS model. 

`tsne_data.txt`: Commands used to collect the data for t-SNE.

`TSNE.ipynb`: Build of all t-SNE models and graphs.

# Original Project Guidelines: DSGA1004 - BIG DATA
## Final project
